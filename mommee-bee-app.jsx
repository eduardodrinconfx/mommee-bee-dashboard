...
// Debe coincidir con la reclasificacion hecha en la base de datos.
...
export default function MommeeBeeApp(props) {
  ...
  function loadData() {
    ...
    Promise.all([
      ...
      function applyExpenses(list) {
        setExpenses(list.map(function(e) { return { id: e.id, date: e.date, category: e.category, desc: e.description || "", amount: parseFloat(e.amount_usd) || 0, type: e.expense_type || classifyExpense(e.category) }; }));
        setLoading(false);
      }
  ...
  var addAbono = function(id, amount) {
    ...
    supabase.from("expenses").insert({ date: today, category: "Evento", description: ev.description + " - Abono", amount_usd: amt, expense_type: "EVENT" }).select().single().then(function(res) {
      if (res.data) setExpenses(function(ex) { return [{ id: res.data.id, date: today, category: "Evento", desc: ev.description + " - Abono", amount: amt, type: "EVENT" }].concat(ex); });
    });
  ...
  var saveExpense = function() {
    ...
    var row = { date: expForm.date, category: expForm.category, description: expForm.desc, amount_usd: amount, expense_type: etype };
    supabase.from("expenses").insert(row).select().single().then(function(res) {
      var d = res.data || {};
      setExpenses(function(ex) { return [{ id: d.id || Date.now(), date: expForm.date, category: expForm.category, desc: expForm.desc, amount: amount, type: etype }].concat(ex); });
  ...
  // COGS = inversion en inventario (no es gasto operativo). EVENT = va al P&L del evento.
